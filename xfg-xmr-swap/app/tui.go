...
func (m *tuiModel) doAccept(idPrefix string) {
	...
	m.status = fmt.Sprintf("SWAP INITIATED — Lock %.6f XMR with hashlock %s...", xmrNeeded, m.swap.hashLock[:16])
	m.tab = 2
}
...
func (m tuiModel) renderHeader() string {
	...
	if m.price != nil {
		...
		if rate > 0 {
			xmrPerXfg := 1.0 / rate
			priceStr = priceStyle.Render(fmt.Sprintf("1 XFG = %.8f XMR", xmrPerXfg))
	...
	title := headerStyle.Render("Ⓜ XFG⇄XMR")
...
func (m tuiModel) renderOrderBook() string {
	...
	hdr := fmt.Sprintf("  %-10s %16s %16s %14s %s", "OFFER", "XFG AMOUNT", "RATE (XFG/XMR)", "XMR VALUE", "AGE")
	...
	for i, o := range m.offers {
		...
		line := fmt.Sprintf("  %-10s %13.2f XFG %13.1f %11.6f XMR %s", id, xfg, rate, xmrVal, age)
...
func (m tuiModel) renderTrades() string {
	...
	hdr := fmt.Sprintf("  %16s %16s %14s %s", "XFG VOLUME", "RATE (XFG/XMR)", "XMR VALUE", "AGE")
	lines := []string{dimStyle.Render(hdr)}
	maxLines := m.height - 8
	if maxLines < 3 {
		maxLines = 3
	...
	for i, t := range m.trades {
		...
		line := fmt.Sprintf("  %13.2f XFG %13.1f %11.6f XMR %s", xfg, rate, xmrVal, age)
...
func (m tuiModel) renderInfo() string {
	...
	if m.swap != nil {
		...
		lines = append(lines, fmt.Sprintf("  XMR needed: %.6f", m.swap.xmrNeeded))
		lines = append(lines, fmt.Sprintf("  Rate:       %.1f XFG/XMR", m.swap.rate))
		lines = append(lines, "")
		...
		if m.swap.xfgLocked {
			...
			lines = append(lines, dimStyle.Render("    1. Lock XMR with hashlock above"))
			lines = append(lines, dimStyle.Render("    2. Maker sees your XMR lock, locks XFG"))
	...
	lines = append(lines, dimStyle.Render("  2. accept <offer_id> → locks your XMR in HTLC"))
	...
	lines = append(lines, dimStyle.Render("  5. Maker claims your XMR with revealed preimage"))
	lines = append(lines, "")
	lines = append(lines, dimStyle.Render("  HEAT/XMR pool:  1 XFG = 10,000,000 HEAT (ERC-20)"))
	return strings.Join(lines, "\n")
}
