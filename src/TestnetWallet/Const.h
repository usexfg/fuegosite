...
namespace {
  ...
  // TESTNET: Use testnet RPC port (28280) instead of mainnet port (18180)
  const command_line::arg_descriptor<uint16_t>    arg_daemon_port = { "daemon-port", "Use daemon instance at port <arg> instead of default", CryptoNote::RPC_DEFAULT_PORT_TESTNET };
  const command_line::arg_descriptor<uint32_t>    arg_log_level = { "set_log", "", Logging::INFO, true };
  const command_line::arg_descriptor<bool>        arg_testnet = { "testnet", "Testnet wallet - always uses testnet mode", false };
