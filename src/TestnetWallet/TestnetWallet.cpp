...
namespace CryptoNote
{
  ...
  void CryptoNote::testnet_wallet::register_testnet_commands()
  {
    ...
    m_consoleHandler.setHandler("register_alias", boost::bind(&testnet_wallet::register_alias, this, boost::arg<1>()), "register_alias <alias> - Register a TEST alias (8 chars ONLY: [A-Z0-9] (CAPS-LOCK) or [a-z0-9] (lowercase))");
  ...
  bool CryptoNote::testnet_wallet::burn(const std::vector<std::string> &args)
  {
    ...
    try
    {
      ...
      success_msg_writer() << "  Banking Fee: " << m_currency.formatAmount(banking_fee) << " TEST";
