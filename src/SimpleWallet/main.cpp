...
// Namespace alias for compatibility with copied code
...
int main(int argc, char* argv[])
{
  ...
  Logging::LoggerRef logger(logManager, "firewallet");
  ...
  bool r = command_line::handle_error_helper(desc_all, [&]()
  {
    ...
    {
      std::cout << FIRE_WALLET_RELEASE_VERSION << std::endl;
  ...
  logger(INFO, BRIGHT_YELLOW) << FIRE_WALLET_RELEASE_VERSION;
  ...
  if (command_line::has_arg(vm, Tools::wallet_rpc_server::arg_rpc_bind_port))
  {
    ...
    std::unique_ptr<cn::IWalletLegacy> wallet(new cn::WalletLegacy(currency, *node.get(), logManager));
  ...
  else
  {
    ...
    cn::simple_wallet cwal(dispatcher, currency, logManager);
