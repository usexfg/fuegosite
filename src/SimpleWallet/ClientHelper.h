...
namespace CryptoNote
{
  ...
  public:
    ...
    **/
    uint32_t deposit_term(const Deposit &deposit) const;

    /**
     * @return - returns deposit amount as string
    **/
    std::string deposit_amount(const Deposit &deposit, const Currency &currency) const;

    /**
     * @return - returns deposit interest based of amount and term as string
    **/
    std::string deposit_interest(const Deposit &deposit, const Currency &currency) const;

    /**
     * @return - "Locked", "Unlocked" or "Spent" depending on state of deposit
    **/
    std::string deposit_status(const Deposit &deposit) const;

    /**
     * @return - returns deposit creatingTransactionId
    **/
    size_t deposit_creating_tx_id(const Deposit &deposit) const;

    /**
     * @return - returns deposit spendingTransactionId
    **/
    size_t deposit_spending_tx_id(const Deposit &deposit) const;

    /**
     * @return - returns unlock height from transaction blockheight + deposit term
    **/
    std::string deposit_unlock_height(const Deposit &deposit, const uint32_t &blockHeight) const;
    ...
    **/
    std::string get_deposit_info(const Deposit &deposit, DepositId did, const Currency &currency, const uint32_t &blockHeight) const;

    /**
     * @return - returns full deposit info string for client output
    **/
    std::string get_full_deposit_info(const Deposit &deposit, DepositId did, const Currency &currency, const uint32_t &blockHeight) const;
    ...
    **/
    ...
    std::string tryToOpenWalletOrLoadKeysOrThrow(Logging::LoggerRef& logger, std::unique_ptr<IWalletLegacy>& wallet, const std::string& walletFile, const std::string& password);

    /**
     * @return - Displays all balances (main + deposits)
     */
    std::stringstream balances(const IWallet &wallet, const Currency &currency) const;
  };
}
