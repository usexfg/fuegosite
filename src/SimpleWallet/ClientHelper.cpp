...
{
  uint32_t client_helper::deposit_term(const Deposit &deposit) const
  {
    return deposit.term;
  }

  std::string client_helper::deposit_amount(const Deposit &deposit, const Currency &currency) const
  {
    return currency.formatAmount(deposit.amount);
  }

  std::string client_helper::deposit_interest(const Deposit &deposit, const Currency &currency) const
  {
    return currency.formatAmount(deposit.interest);
  }

  std::string client_helper::deposit_status(const Deposit &deposit) const
  {
    std::string status_str = "";

    if (deposit.locked)
      status_str = "Locked";
    else if (deposit.spendingTransactionId == WALLET_LEGACY_INVALID_TRANSACTION_ID)
      status_str = "Unlocked";
    else
  ...
  size_t client_helper::deposit_creating_tx_id(const Deposit &deposit) const
  {
    return deposit.creatingTransactionId;
  }

  size_t client_helper::deposit_spending_tx_id(const Deposit &deposit) const
  {
    return deposit.spendingTransactionId;
  }

  std::string client_helper::deposit_unlock_height(const Deposit &deposit, const uint32_t &blockHeight) const
  {
    std::string unlock_str = "";

    if (blockHeight > parameters::CRYPTONOTE_MAX_BLOCK_NUMBER)
    {
  ...
  std::string client_helper::deposit_height(const uint32_t &blockHeight) const
  {
    ...
    bool bad_unlock = blockHeight > parameters::CRYPTONOTE_MAX_BLOCK_NUMBER;
    if (bad_unlock)
    {
  ...
  std::string client_helper::get_deposit_info(const Deposit &deposit, DepositId did, const Currency &currency, const uint32_t &blockHeight) const
  {
  ...
  std::string client_helper::get_full_deposit_info(const Deposit &deposit, DepositId did, const Currency &currency, const uint32_t &blockHeight) const
  {
    std::stringstream full_info;
  ...
  std::string client_helper::list_tx_item(const WalletTransaction& txInfo, std::string listed_tx, const Currency &currency) const
  {
    ...
    std::string paymentIdStr = (getPaymentIdFromTxExtra(extraVec, paymentId) && paymentId != NULL_HASH ? podToHex(paymentId) : "");
  ...
  std::error_code client_helper::initAndLoadWallet(IWalletLegacy& wallet, std::istream& walletFile, const std::string& password)
  {
  ...
  std::string client_helper::tryToOpenWalletOrLoadKeysOrThrow(Logging::LoggerRef& logger, std::unique_ptr<IWalletLegacy>& wallet, const std::string& walletFile, const std::string& password)
  {
  ...
  std::stringstream client_helper::balances(const IWallet &wallet, const Currency &currency) const
  {
