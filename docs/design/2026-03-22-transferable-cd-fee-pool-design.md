...
2. **Fee Pool** — Across all adaptor-sig swaps (not just HTLC), there is a total 2% swap-fee lifecycle: 1% is added when Bob initiates a transfer (TransactionInputCommitmentTransfer) to the fee pool, and 1% is subtracted from the claimer's claim input (TransactionInputCommitmentSpend) into the same per-epoch fee pool accumulator. Miner reward remains unaffected.
...
**Fee source:** Every adaptor-sig swap claim/refund and initiation contribute to the fee pool. The per-event fee is governed by the rate SWAP_FEE_RATE_BPS / SWAP_FEE_RATE_DIVISOR (1% per event with SWAP_FEE_RATE_BPS = 100, SWAP_FEE_RATE_DIVISOR = 10000).

```
swap_fee_event = amount * SWAP_FEE_RATE_BPS / SWAP_FEE_RATE_DIVISOR
```

For a 100 XFG event, swap_fee_event ≈ 1 XFG per event (assuming 1%). Across initiation and claim, total 2 XFG would flow to the fee pool for two events.
...
```
Adaptor-sig initiation (Bob): 100.0000000 XFG deposited; fee_pool += 1.0000000 XFG
 adaptor-sig claim (Alice): Claimer receives 99.0000000 XFG; fee_pool += 1.0000000 XFG
...
```cpp
// CryptoNoteConfig.h
const uint64_t SWAP_FEE_RATE_BPS = 100;               // 1% (per event)
...
```
Inputs:  CommitmentTransfer (seller's CD) + KeyInput 
(buyer's liquid XFG)
Outputs: new CD (buyer's commitKey, term=newTerm) + KeyOutput 
(seller receives liquid XFG)
```

Both parties must co-sign for v1, manual peer-to-peer. Explorer-served orderbook is a follow-on.
